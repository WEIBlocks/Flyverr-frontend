...
import { Package, User, Clock, CheckCircle, XCircle, Eye, AlertCircle, TrendingUp, FileText, Calendar, Zap, Filter, Loader2, XCircle as XCircleIcon } from "lucide-react"
...
export default function AdminPendingProductsPage() {
  ...
  const { data, isLoading, error } = useGetPendingProducts({ page: currentPage, limit: currentLimit })
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      default:
        return <Badge className="bg-gray-100 text-gray-800 dark:bg-gray-900/30 dark:border-gray-800">{status}</Badge>
    }
  }

  const getStageBadge = (stage: string) => {
    const stageColors = {
      newboom: "bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400 border-green-200 dark:border-green-800",
      blossom: "bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400 border-blue-200 dark:border-blue-800",
      evergreen: "bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-400 border-purple-200 dark:border-purple-800",
      exit: "bg-orange-100 text-orange-800 dark:bg-orange-900/30 dark:text-orange-400 border-orange-200 dark:border-orange-800"
    }
    
    return (
      <Badge className={stageColors[stage as keyof typeof stageColors] || "bg-gray-100 text-gray-800 dark:bg-gray-900/30 dark:text-gray-400 border-gray-200 dark:border-gray-800"}>
        {stage}
      </Badge>
    )
  }
        ...
        <div className="flex items-center justify-center py-12">
          <div className="text-center">
            ...
            <p className="text-gray-600 dark:text-gray-400 mb-6">
              {error instanceof Error ? error.message : 'Failed to load pending products'}
            </p>
            <Button onClick={() => window.location.reload()} className="bg-flyverr-primary hover:bg-flyverr-primary/90">
              Try Again
            </Button>
          </div>
        </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Pending Products</h1>
          <p className="text-gray-600 dark:text-gray-400 mt-2">Review and approve new product submissions</p>
        </div>
        <div className="flex items-center space-x-3">
          <Button variant="outline" className="border-flyverr-primary text-flyverr-primary hover:bg-flyverr-primary/10">
        ...
        <div className="bg-white dark:bg-gray-800 p-6 rounded-xl border-2 border-gray-100 dark:border-gray-700 shadow-lg">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-gray-900 dark:text-white">{pagination?.total || 0}</p>
            </div>
        ...
        <div className="bg-white dark:bg-gray-800 p-6 rounded-xl border-2 border-gray-100 dark:border-gray-700 shadow-lg">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-gray-900 dark:text-white">
                {new Set(pendingProducts.map(p => p.category?.name || 'Uncategorized')).size}
              </p>
            </div>
