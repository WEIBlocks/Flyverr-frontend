...
export default function StaleResalesPage() {
  ...
  const { data: insuranceData, isLoading, isFetching, error, refetch } =
    useGetExpiredInsurance({ status, includeResold, page, limit })
  const [selectedRecord, setSelectedRecord] = useState<string | null>(null)
  const [isDetailOpen, setIsDetailOpen] = useState(false)

  const handlePageChange = (nextPage: number) => {
    setPage(() => Math.max(1, Math.min(nextPage, insuranceData?.data?.pagination.totalPages || 1)))
  }
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-semibold text-gray-900 dark:text-white">Insurance Management</h2>
            <p className="text-gray-600 dark:text-gray-400 mt-1">Manage expired insurance and overdue resales</p>
          </div>
          <Button onClick={() => refetch()} className="bg-flyverr-primary hover:bg-flyverr-primary/90 text-white">
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-center py-12">
              <div className="text-center">
                ...
                <p className="text-gray-500 text-sm mt-2 dark:text-gray-400">Please try again later</p>
              </div>
            </div>
          </CardContent>
        </Card>
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center gap-4">
          <div className="flex items-center gap-2">
            ...
            <div className="flex rounded-md overflow-hidden border border-gray-200 dark:border-gray-700">
              {(["all", "active", "expired"] as InsuranceStatusFilter[]).map((s) => (
                <button
                  key={s}
                  onClick={() => {
                    setPage(1); setStatus(s);
                  }}
          ...
          <label className="inline-flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
            <input
              ...
              onChange={(e) => { setPage(1); setIncludeResold(e.target.checked); }}
      ...
      {isInitialLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {[1,2,3,4].map((i) => (
            <Card key={i} className="bg-white dark:bg-gray-800 p-6 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm">
            ...
            <CardContent className="p-0">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-orange-600 dark:text-orange-400">{summary.overdue}</p>
                </div>
            ...
            <CardContent className="p-0">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-green-600 dark:text-green-400">{formatCurrency(summary.totalInsuranceFees)}</p>
                </div>
        ...
        <CardContent className="p-0">
          {isInitialLoading ? (
            <div className="p-6">
              ...
              {[1,2,3,4,5].map((i) => (
                <div key={i} className="h-10 bg-gray-200 dark:bg-gray-700 rounded-md animate-pulse mb-2" />
              ))}
            </div>
          ) : records.length === 0 ? (
                ...
                <tbody>
                  {records.map((record) => (
                    <tr 
                      ...
                      onClick={() => { setSelectedRecord(record.id); setIsDetailOpen(true); }}
                    >
                      ...
                      <td className="px-4 py-3">
                        <div className="flex space-x-2">
                          ...
                          <Button 
                            size="sm" 
                            variant="outline"
                            className="dark:text-gray-100 bg-flyverr-primary hover:bg-flyverr-primary/90 text-white"
