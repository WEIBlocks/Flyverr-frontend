...
import { Package, User, DollarSign, TrendingUp, Eye, Edit, MoreVertical, Star, ShoppingCart, BarChart3, Activity, Target, TrendingDown, Search, Filter, Calendar, FileText, Clock, Loader2, AlertCircle } from "lucide-react"
...
export default function AdminAllProductsPage() {
  ...
  const { data, isLoading, error } = useGetAllProducts(page, limit, status, debouncedSearch)
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'pending':
        return <Badge className="bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-green-200 dark:border-green-800">Pending</Badge>
      case 'rejected':
      ...
      case 'paused':
        return <Badge className="bg-orange-100 text-orange-800 dark:bg-orange-900/30 dark:text-orange-400 border-orange-200 dark:border-orange-800">Paused</Badge>
      default:
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          ...
          <p className="text-gray-600 dark:text-gray-400">Failed to load products. Please try again.</p>
        </div>
      </div>
    )
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-2">Manage and monitor all platform products</p>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          >
            <div className="flex items-center space-x-2">
              <div className={`w-2 h-2 rounded-full ${status === "approved" ? "bg-white" : "bg-green-500"}`}></div>
              <span>Approved</span>
            </div>
          </button>
          <button
          ...
          >
            <div className="flex items-center space-x-2">
              <div className={`w-2 h-2 rounded-full ${status === "pending" ? "bg-white" : "bg-yellow-500"}`}></div>
              <span>Pending</span>
            </div>
          </button>
          <button
          ...
          >
            <div className="flex items-center space-x-2">
              <div className={`w-2 h-2 rounded-full ${status === "rejected" ? "bg-white" : "bg-red-500"}`}></div>
              <span>Rejected</span>
            </div>
          </button>
        </div>
                  ...
                  >
                    ...
                    <AdminTableCell>
                      <div className="space-y-1">
                        ...
                        <div className="text-xs text-gray-500 dark:text-gray-400">
                          {product.approved_at ? `Approved: ${formatDate(product.approved_at)}` : 'Not approved yet'}
                        </div>
                      </div>
                    </AdminTableCell>
