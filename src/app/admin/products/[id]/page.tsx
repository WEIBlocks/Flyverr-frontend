...
const editProductSchema = yup.object({
  ...
  original_price: yup.number().positive('Price must be positive').required('Price is required'),
  total_licenses: yup.number().integer('Licenses must be a whole number').positive('Licenses must be positive').required('Total licenses is required'),
  featured: yup.boolean().required(),
  adminNotes: yup.string().optional(),
  roundPricing: yup.object({
    exit_price: yup.number().positive('Exit price must be positive').required('Exit price is required'),
    blossom_price: yup.number().positive('Blossom price must be positive').required('Blossom price is required'),
    evergreen_price: yup.number().positive('Evergreen price must be positive').required('Evergreen price is required'),
  }).required()
})
...
export default function AdminProductDetailPage() {
  ...
  const [flagAction, setFlagAction] = useState<'flag' | 'unflag' | 'delete'>('flag')
  const [flagReason, setFlagReason] = useState('')
  const [flagType, setFlagType] = useState<'inappropriate' | 'copyright' | 'spam' | 'quality' | 'other' | ''>('')
  ...
  const handleFlagProduct = async () => {
    try {
      ...
      const actionText = flagAction === 'flag' ? 'hidden' : flagAction === 'unflag' ? 'shown' : 'deleted'
      ...
      const actionText = flagAction === 'flag' ? 'hide' : flagAction === 'unflag' ? 'show' : 'delete'
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'pending':
        return <Badge className="bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400 border-yellow-200 dark:border-yellow-400">Pending</Badge>
      case 'rejected':
      ...
      case 'flagged':
        return <Badge className="bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-400 border-amber-200 dark:border-amber-800">Hidden</Badge>
      case 'deleted':
        return <Badge className="bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400 border-red-200 dark:border-red-800">Deleted</Badge>
      default:
    ...
    return (
      <div className="min-h-screen bg-flyverr-neutral dark:bg-gray-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <Button onClick={() => router.push('/admin/products')} className="bg-flyverr-primary hover:bg-flyverr-primary/90">
            Back to Products
          </Button>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-flyverr-neutral dark:bg-gray-900">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 lg:px-12 xl:px-16 py-4 sm:py-6 md:py-8 lg:py-12 xl:py-16">
         ...
         <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 mb-8">
          ...
          <div>
            ...
            <p className="text-gray-600 dark:text-gray-400 mt-2">
              {product.status === 'deleted' ? 'This product has been deleted' : 'View and edit product information'}
            </p>
          </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className={`space-y-8 ${product.status === 'deleted' ? 'opacity-50 pointer-events-none' : ''}`}>
                  ...
                  <Label className="text-sm font-medium text-gray-700 dark:text-gray-300">
                   ...
                   <div className="space-y-3">
                     ...
                     {errors.thumbnail_url && (
                       <p className="text-sm text-red-600 dark:text-red-400">{errors.thumbnail_url.message}</p>
                  )}
                </div>
                  ...
                  <div className="space-y-2">
                    <Label className="text-sm font-medium text-gray-700 dark:text-gray-300">
                      ...
                      <Input
                        ...
                        className={`bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 ${
                          errors.original_price ? 'border-red-500 focus:border-red-500' : ''
                        }`}
                      />
                      {errors.original_price && (
                        <p className="text-sm text-red-600 dark:text-red-400">{errors.original_price.message}</p>
                    )}
                  </div>
                  <div className="space-y-2">
                    <Label className="text-sm font-medium text-gray-700 dark:text-gray-300">
                      ...
                      <Input
                        ...
                        className={`bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 ${
                          errors.total_licenses ? 'border-red-500 focus:border-red-500' : ''
                        }`}
                      />
                      {errors.total_licenses && (
                        <p className="text-sm text-red-600 dark:text-red-400">{errors.total_licenses.message}</p>
                    )}
                  </div>
                </div>
                  ...
                  <div className="space-y-2">
                    <Label className="text-sm font-medium text-gray-700 dark:text-gray-300">
                      ...
                      <Input
                        type="number"
                        step="0.01"
                        {...register('roundPricing.exit_price', { valueAsNumber: true })}
                        className={`bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 ${
                          errors.roundPricing?.exit_price ? 'border-red-500 focus:border-red-500' : ''
                        }`}
                      />
                      {errors.roundPricing?.exit_price && (
                        <p className="text-sm text-red-600 dark:text-red-400">{errors.roundPricing.exit_price.message}</p>
                    )}
                  </div>
                  <div className="space-y-2">
                    <Label className="text-sm font-medium text-gray-700 dark:text-gray-300">
                      ...
                      <Input
                        type="number"
                        step="0.01"
                        {...register('roundPricing.blossom_price', { valueAsNumber: true })}
                        className={`bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 ${
                          errors.roundPricing?.blossom_price ? 'border-red-500 focus:border-red-500' : ''
                        }`}
                      />
                      {errors.roundPricing?.blossom_price && (
                        <p className="text-sm text-red-600 dark:text-red-400">{errors.roundPricing.blossom_price.message}</p>
                    )}
                  </div>
                  <div className="space-y-2">
                    <Label className="text-sm font-medium text-gray-700 dark:text-gray-300">
                      ...
                      <Input
                        type="number"
                        step="0.01"
                        {...register('roundPricing.evergreen_price', { valueAsNumber: true })}
                        className={`bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 ${
                          errors.roundPricing?.evergreen_price ? 'border-red-500 focus:border-red-500' : ''
                        }`}
                      />
                      {errors.roundPricing?.evergreen_price && (
                        <p className="text-sm text-red-600 dark:text-red-400">{errors.roundPricing.evergreen_price.message}</p>
                    )}
                  </div>
                </div>
              </CardContent>
             ...
             <Card className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-sm hover:shadow-md transition-shadow duration-200">
              <CardContent className="p-6">
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-medium text-gray-600 dark:text-gray-400">Round</span>
                    <span className="text-sm font-semibold text-gray-900 dark:text-white">{product.current_round}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-medium text-gray-600 dark:text-gray-400">Category</span>
                    <span className="text-sm font-semibold text-gray-900 dark:text-white">{product.category?.name}</span>
                  </div>
                </div>
              </CardContent>
            </Card>
                ...
                <div className="flex items-center space-x-3">
                  <div className="w-10 h-10 bg-flyverr-primary/20 rounded-full flex items-center justify-center">
                    <span className="text-flyverr-primary font-semibold">
                      {product.creator.first_name.charAt(0)}{product.creator.last_name.charAt(0)}
                    </span>
                  </div>
                  <div>
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-400">@{product.creator.username}</p>
                  </div>
                </div>
             ...
             <Card className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-sm hover:shadow-md transition-shadow duration-200">
              ...
              <CardContent className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm font-semibold text-gray-900 dark:text-white">{product.file_type}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-gray-600 dark:text-gray-400">Size</span>
                  <span className="text-sm font-semibold text-gray-900 dark:text-white">{formatFileSize(product.file_size)}</span>
                </div>
                <div className="flex items-center justify-between">
             ...
             <Card className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-sm hover:shadow-md transition-shadow duration-200">
              ...
              <CardContent className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm font-semibold text-gray-900 dark:text-white">{statistics.totalLicenses}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-gray-600 dark:text-gray-400">Sold Licenses</span>
                  <span className="text-sm font-semibold text-gray-900 dark:text-white">{statistics.soldLicenses}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-gray-600 dark:text-gray-400">Total Revenue</span>
                  <span className="text-sm font-semibold text-gray-900 dark:text-white">${statistics.totalRevenue}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-gray-600 dark:text-gray-400">Transactions</span>
                  <span className="text-sm font-semibold text-gray-900 dark:text-white">{statistics.totalTransactions}</span>
                </div>
              </CardContent>
            </Card>
             ...
             <Card className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-sm hover:shadow-md transition-shadow duration-200">
              ...
              <CardContent className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm font-semibold text-gray-900 dark:text-white">{formatDate(product.created_at)}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-gray-600 dark:text-gray-400">Updated</span>
                  <span className="text-sm font-semibold text-gray-900 dark:text-white">{formatDate(product.updated_at)}</span>
                </div>
              </CardContent>
            </Card>
          </div>
             ...
             <div className="space-y-4">
               <div>
                 ...
                 <Textarea
                   ...
                   placeholder={`Enter reason for ${flagAction === 'flag' ? 'hiding' : flagAction === 'unflag' ? 'showing' : 'deleting'} this product...`}
                   rows={3}
                 ...
                 <div>
                   <Label className="text-sm font-medium text-gray-700 dark:text-gray-300">
                     Flag Type *
                                        ...
                                        <select
                       value={flagType}
                       onChange={(e) => setFlagType(e.target.value as 'inappropriate' | 'copyright' | 'spam' | 'quality' | 'other' | '')}
               ...
               <div className="flex items-center justify-end space-x-3 pt-4">
                 ...
                 <Button
                   ...
                   disabled={!flagReason.trim() || (flagAction === 'flag' && !flagType) || flagProductMutation.isPending}
                   className={`${
                     flagAction === 'delete' 
                 ...
                 >
                   {flagProductMutation.isPending ? (
                     <>
                       <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white mr-2"></div>
                       {flagAction === 'delete' ? 'Deleting...' : flagAction === 'flag' ? 'Hiding...' : 'Showing...'}
                     </>
                   ) : (
                     <>
                       {flagAction === 'delete' && <Trash2 className="w-4 h-4 mr-2" />}
                       {flagAction === 'flag' && <Flag className="w-4 h-4 mr-2" />}
                       {flagAction === 'unflag' && <Shield className="w-4 h-4 mr-2" />}
                       {flagAction === 'delete' ? 'Delete Product' : flagAction === 'flag' ? 'Hide Product' : 'Show Product'}
                     </>
                   )}
                 </Button>
               </div>
