...
const productSchema = yup.object({
  ...
  originalPrice: yup.string().required("Price is required").matches(/^\d+(\.\d{1,2})?$/, "Price must be a valid number"),
  totalLicenses: yup.string().required("Total licenses is required").matches(/^\d+$/, "Must be a valid number"),
  thumbnailUrl: yup.string().required("Thumbnail is required"),
  imagesUrls: yup.array().of(yup.string().required()).min(2, "At least 2 additional images are required").required(),
...
export default function AddProductModal({
  ...
  const removeImage = (index: number) => {
    ...
    if (newImages.length < 2) {
      setError("imagesUrls", { message: "At least 2 additional images are required" });
    }
  };
  ...
  const onSubmit = (data: NewProduct) => {
    ...
    createProduct(data, {
      ...
      onError: (error: any) => {
        setApiError(error);
        toast.error("Failed to create product. Please check the errors below.");
      }
    });
  };
        ...
        <form onSubmit={handleSubmit((data) => onSubmit(data))} className="space-y-6">
          ...
          */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              {errors.totalLicenses && (
                <p className="text-sm text-destructive">{errors.totalLicenses.message}</p>
              )}
            </div>
          </div>
              ...
              <div className="border border-border rounded-lg p-4 bg-muted/50">
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <p className="font-medium text-foreground">{thumbnailFile.name}</p>
          ...
          <div className="space-y-2">
            ...
            {imageFiles.length > 0 && (
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 mb-3">
                {imageFiles.map((file, index) => (
                  <div key={index} className="relative border border-border rounded-lg p-3 bg-muted/50 group">
                    ...
                    <div className="text-center">
                      <p className="text-xs font-medium text-foreground truncate mb-1">{file.name}</p>
            ...
            />
            {errors.imagesUrls && (
              <p className="text-sm text-destructive">{errors.imagesUrls.message}</p>
            )}
          </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    <div className="w-12 h-12 bg-flyverr-primary/20 rounded-lg flex items-center justify-center">
                      ...
                      ) : mainFile.type.includes('zip') || mainFile.type.includes('rar') ? (
                        <div className="text-flyverr-primary font-bold text-lg">📦</div>
                    ...
                    <div>
                      <p className="font-medium text-foreground">{mainFile.name}</p>
                      <p className="text-sm text-muted-foreground">
                        {(mainFile.size / 1024 / 1024).toFixed(2)} MB • {mainFile.type || 'Unknown type'}
                      </p>
                    </div>
                  </div>
                  <Button
